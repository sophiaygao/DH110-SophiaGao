...
### Background Info
BeReal is a unique social media platform where every day at a different time, everyone is notified at the same time to capture and share a picture in 2 minutes. It's a new way to discover who your friends are in their daily life. 

### Heuristic Evaluation
Overall Evaluation:
The platform is definitely a unique way for users to see what's up at any given point in the day, but isn't really a good way for people to see memories or updates long term. It only shows what that user is doing at that specific time, and is removed once the next BeReal alarm goes off. This app wouldn't be a good way for families to stay connected. 

#### 1. Visibility of System Status
The application allows the user to know if they've taken their BeReal or not, as in order to view their feed they need to have taken one. I think it's a really clear way to notify the users of their usage of the app that day or not, but sometimes people might not be in the mood to take one and just want to view others (which defeats the purpose of the app in a way). 
Severity: 1

#### 2. Match Between System & Real World
It uses clear and concise language that is easy to navigate through without a problem. 
Severity: 0

#### 3. User Control and Freedom
Allows users to undo any BeReals or comments they took or made. When reacting to someone else's BeReal, users can't remove an existing one, they're only allowed to replace it. This could be a problem if they reacted so someone's BeReal by accident. 
Severity: 2

#### 4. Consistency and Standards
This platform isn't really like others, so it needed some time to get used to, but after using it for a while, it is pretty easy and fun to use. 
Severity: 2

#### 5. Error Prevention
Allows me to confirm if I want to retake a BeReal or not, which is the main source of "error" on this app. 
Severity: 0

#### 6. Recognition Rather Than Recall
There is only one main feed page, and then the users profile which contains BeReals from the past. There is not much recognition needed from the users.
Severity: 1

#### 7. Flexibility and Efficiency of Use
No shortcuts on the app, but is pretty efficient to use since all you have to do is click a button every day.
Severity: 2

#### 8. Aesthetic and Minimalistic Design
Not super aesthetic, but the design is simple, straightforward, and gets the job done. There isn't much to the app function, so not much design is needed I guess.
Severity: 2

#### 9. Help Users with Errors
Not really... the app is pretty simple and basically only has one function, so the app isn't really built to handle a lot of errors. 
Severity: 2

#### 10. Help and Documentation
There is a help tab when navigated from the users Profile page. There listed are resources for the user to report any issues encountered, as well as contacting the company.
Severity: 1
